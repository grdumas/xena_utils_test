...
def main(args):
    ...
    try:
        ...
        print(e)
    try:
        # add a signle stream and configure
        ...
        #print(stats.items()) # debug

        pkt_sent = args.duration * args.pps
        print('Packets sent: {}'.format(pkt_sent))
        
        #pkt_rec = stat.data['pr_total']['packets'] #original
        #pkt_rec = stats.get(['pr_total']['packets']) #throws exception
        pr_total = stats.get('pr_total')
        pkt_rec = pr_total.get('packets')
        
        #pkt_latency_avg = stat.data['pr_tpldlatency']['1']['avg']
        #pkt_latency_avg = stats.get(['pr_tpldlatency']['1']['avg']) #exception: list indicies must be integers or slices, not str
        ...
        pkt_lost = pkt_sent - pkt_rec # both original and refactor return negative numbers here. Is there a better stat I can use?
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--duration', type=int,
                        required=False, default=1, help='Duration to run')
