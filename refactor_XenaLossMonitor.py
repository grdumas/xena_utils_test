...
def main(args):
    ...
    xena_socket.connect()
    #time.sleep(1)
    #xena_manager = XenaManager(xena_socket, 'Monitor')
    ...
    while totaltime < args.length:
        ...
        if totaltime % args.interval == 0:
            ...
            #rx_stats0 = port0.dump_all_rx_stats()
            p0_errors = port0.get_total_errors_counter()

            #rx_stats1 = port1.get_rx_stats()
            port1.grab_all_rx_stats()
            #rx_stats1 = port0.dump_all_rx_stats()
            p1_errors = port1.get_total_errors_counter()

            _LOGGER.info('Port 0 total lost frames: {}'.format(p0_errors))
            _LOGGER.info('Port 1 total lost frames: {}'.format(p1_errors))
